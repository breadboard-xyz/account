...
app.get('/account/:id/token/', (req, res, next) => {
...
app.get('/account/:id/secret/:secret/token', (req, res, next) => {
  var id      = req.params['id']
  ...
  User
    ...
    .then((account) => {
      let salt = account.salt
        , hash = crypto.createHash('sha1', salt).update(secret).digest('hex')
        ;

      if (!_.includes(account.secrets, hash)) {
